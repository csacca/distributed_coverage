...
# Find each's nodes' isolated optimal angle
for i in range(0, len(nodes)):
    maxtemp=0;
    temp=numpy.arange(anglemin,anglemax+increm,increm)
    for angle in temp:
        nodetemp=[]
        nodetemp.append(nodes[len(nodes)-i-1])
        angletemp=[]
        angletemp.append(angle)
        percentinit=converge(xmin,xmax,ymin,ymax,fovangle,nodetemp,angletemp)
        if(percentinit > maxtemp):
            maxtemp=percentinit
            tiltnew=angle
    tilt[len(nodes)-i-1]=tiltnew
